...
public class SQLTgacSubmissionDAO implements Store<Submission>, NamingSchemeAware<Submission> {
    ...
    public static final String SUBMISSION_DILUTION_SELECT = "SELECT dilution_dilutionId " +
                ...
                if (namingScheme.validateField("name", submission.getName())) {
                    ...
                    if (newId.longValue() != submission.getId()) {
                        ...
                        new NamedParameterJdbcTemplate(template)
                            .update(SUBMISSION_DELETE, new MapSqlParameterSource().addValue("submissionId", newId.longValue()));
                    ...
                    if (l.getPool() != null) {
                        ...
                        for (Dilution dil : dils) {
                            ...
                            try {
                                MapSqlParameterSource poParams = new MapSqlParameterSource();
                                poParams.addValue("submission_submissionId", submission.getId())
                                        .addValue("partition_partitionId", l.getId()).addValue("dilution_dilutionId", dil.getId());
                                sInsert.execute(poParams);

                            } catch (DuplicateKeyException dke) {
                                log.warn(
                                    "This Submission_Partition_Dilution combination already exists - not inserting: " + dke.getMessage());
                            }
                        }
                    }
                }
