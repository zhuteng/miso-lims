...
public class SQLSampleDAO implements SampleStore {
    ...
    public static final String SAMPLES_SELECT_BY_SEARCH = SAMPLES_SELECT + " WHERE " +
    ...
    public int[] batchSave(final Collection<Sample> samples) throws IOException {
        ...
        try {
            for (Sample sample : samples) {
                ...
                if (this.cascadeType !=
                    null) {// && this.cascadeType.equals(CascadeType.PERSIST) || this.cascadeType.equals(CascadeType.REMOVE)) {
                ...
                params.addValue("alias", sample.getAlias()).addValue("accession", sample.getAccession())
                      .addValue("description", sample.getDescription()).addValue("scientificName", sample.getScientificName()).addValue(
                    "taxonIdentifier", sample.getTaxonIdentifier())
                    ...
                    if (sampleNamingScheme.validateField("name", sample.getName()) &&
                        ...
                        if (newId.longValue() != sample.getId()) {
                            ...
                            new NamedParameterJdbcTemplate(template)
                                .update(SAMPLE_DELETE, new MapSqlParameterSource().addValue("sampleId", newId.longValue()));
