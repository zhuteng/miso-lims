...
public class SQLSequencerPartitionContainerDAO implements SequencerPartitionContainerStore {
    ...
    private static final String SEQUENCER_PARTITION_CONTAINER_SELECT =
        "SELECT containerId, platform, identificationBarcode, locationBarcode, validationBarcode, securityProfile_profileId FROM " +
        TABLE_NAME;
    ...
    public static final String SEQUENCER_PARTITION_CONTAINER_PARTITION_DELETE_BY_SEQUENCER_PARTITION_CONTAINER_ID =
        "DELETE FROM SequencerPartitionContainer_Partition " + "WHERE container_containerId=:container_containerId";

    public static final String RUN_SEQUENCER_PARTITION_CONTAINER_DELETE_BY_SEQUENCER_PARTITION_CONTAINER_ID =
        "DELETE FROM Run_SequencerPartitionContainer " + "WHERE containers_containerId=:containers_containerId";
    ...
    public SequencerPartitionContainer<SequencerPoolPartition> get(long sequencerPartitionContainerId) throws IOException {
        ...
        SequencerPartitionContainer<SequencerPoolPartition> f = eResults.size() > 0 ?
                                                                (SequencerPartitionContainer<SequencerPoolPartition>) eResults.get(0) :
                                                                null;
        fillInRun(f);
        return f;
    }
    ...
    public List<SequencerPartitionContainer<SequencerPoolPartition>> listSequencerPartitionContainersByBarcode(String barcode)
        ...
        List<SequencerPartitionContainer<SequencerPoolPartition>> lp = template
            .query(SEQUENCER_PARTITION_CONTAINER_SELECT_BY_IDENTIFICATION_BARCODE, new Object[] { barcode },
                   new SequencerPartitionContainerMapper(true));
        for (SequencerPartitionContainer<SequencerPoolPartition> f : lp) {
            fillInRun(f);
        }
    ...
    public List<SequencerPartitionContainer<SequencerPoolPartition>> listAllSequencerPartitionContainersByRunId(long runId)
        throws IOException {
        List<SequencerPartitionContainer<SequencerPoolPartition>> lp = template
            .query(SEQUENCER_PARTITION_CONTAINER_SELECT_BY_RELATED_RUN, new Object[] { runId },
                   new SequencerPartitionContainerMapper(true));
        for (SequencerPartitionContainer<SequencerPoolPartition> f : lp) {
            fillInRun(f, runId);
        }
  ...
  */
    ...
    public boolean remove(SequencerPartitionContainer container) throws IOException {
        ...
        if (container.isDeletable() && (namedTemplate.update(SEQUENCER_PARTITION_CONTAINER_DELETE,
                                                             new MapSqlParameterSource().addValue("containerId", container.getId())) ==
                                        1)) {
