...
public class SQLStudyDAO implements StudyStore {
    ...
    public static final String STUDIES_SELECT_BY_SEARCH = STUDIES_SELECT + " WHERE " +
                                                          "name LIKE ? OR " +
                                                          "alias LIKE ? OR " +
                                                          "description LIKE ? ";
    ...
    public static final String STUDIES_BY_RELATED_LIBRARY = "SELECT " +
                                                            "stu.* FROM Study stu " +
                ...
                if (namingScheme.validateField("name", study.getName())) {
                    ...
                    if (newId.longValue() != study.getId()) {
                        ...
                        new NamedParameterJdbcTemplate(template)
                            .update(STUDY_DELETE, new MapSqlParameterSource().addValue("studyId", newId.longValue()));
    ...
    public boolean remove(Study study) throws IOException {
        ...
        if (study.isDeletable() &&
            (namedTemplate.update(STUDY_DELETE, new MapSqlParameterSource().addValue("studyId", study.getId())) == 1)) {
            Project p = study.getProject();
            if (this.cascadeType.equals(CascadeType.PERSIST)) {
                if (p != null)
                    projectDAO.save(p);
