...
public class SQLProjectDAO implements ProjectStore {
    ...
    public static final String PROJECTS_SELECT_BY_SEARCH = PROJECTS_SELECT + " WHERE " +
                                                           "name LIKE ? OR " +
                                                           "alias LIKE ? OR " +
                                                           "description LIKE ? ";
    ...
    //OVERVIEWS
    public static final String OVERVIEWS_SELECT = "SELECT p.project_projectId, " +
                                                  "po.overviewId, " +
    ...
    public static String SAMPLES_BY_PROJECT_ID = "SELECT sa.* " +
                                                 "FROM " + TABLE_NAME + " p " +
    ...
    @Deprecated
    public static final String OVERVIEW_RELATED_INFORMATION_BY_PROJECT_ID = "SELECT " +
                                                                            "p.projectId, " +
                                                                            "st.studyId, " +
                                                                            "ex.experimentId, " +
                                                                            "sa.sampleId, " +
                ...
                if (namingScheme.validateField("name", project.getName())) {
                    ...
                    if (newId.longValue() != project.getId()) {
                        ...
                        new NamedParameterJdbcTemplate(template)
                            .update(PROJECT_DELETE, new MapSqlParameterSource().addValue("projectId", newId.longValue()));
    ...
    public ProjectOverview lazyGetProjectOverviewById(long overviewId) throws IOException {
        List<ProjectOverview> eResults = template
            .query(OVERVIEW_SELECT_BY_ID, new Object[] { overviewId }, new ProjectOverviewMapper(true));
        return eResults.size() > 0 ? eResults.get(0) : null;
    }
