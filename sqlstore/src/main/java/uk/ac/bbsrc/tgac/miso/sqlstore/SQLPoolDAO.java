...
public class SQLPoolDAO implements PoolStore {
    ...
    public static final String POOL_SELECT_BY_PLATFORM_AND_SEARCH = POOL_SELECT + " WHERE platformType=? AND " +
                                                                    "(name LIKE ? OR " +
                                                                    "alias LIKE ? OR " +
  ...
  */
    ...
    public static final String EMPCR_POOL_SELECT_BY_RELATED_PROJECT = "SELECT DISTINCT pool.* " +
                                                                      "FROM Project p " +
                                                                      "INNER JOIN Sample sa ON sa.project_projectId = p.projectId " +
                                                                      "INNER JOIN Library li ON li.sample_sampleId = sa.sampleId " +
                                                                      "INNER JOIN LibraryDilution ld ON ld.library_libraryId = li.libraryId " +
                                                                      ...
                                                                      " pool ON pool.poolId = ple.pool_poolId " +
                                                                      "WHERE p.projectId = ? AND ple.elementType = 'uk.ac.bbsrc.tgac.miso.core.data.impl.emPCRDilution'";

    public static final String DILUTION_POOL_SELECT_BY_RELATED_PROJECT = "SELECT DISTINCT pool.* " +
                                                                         "FROM Project p " +
                                                                         "INNER JOIN Sample sa ON sa.project_projectId = p.projectId " +
                                                                         "INNER JOIN Library li ON li.sample_sampleId = sa.sampleId " +
                                                                         "INNER JOIN LibraryDilution ld ON ld.library_libraryId = li.libraryId " +
                                                                         ...
                                                                         "INNER JOIN " + TABLE_NAME +
                                                                         " pool ON pool.poolId = pld.pool_poolId " +
                                                                         "WHERE p.projectId = ? AND pld.elementType = 'uk.ac.bbsrc.tgac.miso.core.data.impl.LibraryDilution'";

    public static final String PLATE_POOL_SELECT_BY_RELATED_PROJECT = "SELECT DISTINCT pool.* " +
                                                                      "FROM Project p " +
                                                                      "INNER JOIN Sample sa ON sa.project_projectId = p.projectId " +
                                                                      "INNER JOIN Library li ON li.sample_sampleId = sa.sampleId " +
    ...
    public static final String POOL_SELECT_BY_RELATED_LIBRARY = "SELECT DISTINCT pool.* " +
                                                                "FROM Library li " +
                                                                "INNER JOIN LibraryDilution ld ON ld.library_libraryId = li.libraryId " +
    ...
    public static final String POOL_SELECT_BY_RELATED_SAMPLE = "SELECT DISTINCT pool.* " +
                                                               "FROM Sample s " +
    ...
    public long save(Pool<? extends Poolable> pool) throws IOException {
                ...
                if (namingScheme.validateField("name", pool.getName())) {
                    ...
                    if (newId.longValue() != pool.getId()) {
                        ...
                        new NamedParameterJdbcTemplate(template)
                            .update(POOL_DELETE, new MapSqlParameterSource().addValue("poolId", newId.longValue()));
        ...
        if (pool.getPoolableElements() != null && !pool.getPoolableElements().isEmpty()) {
            ...
            for (Poolable d : pool.getPoolableElements()) {
                ...
                esParams.addValue("elementId", d.getId()).addValue("pool_poolId", pool.getId())
                ...
                if (this.cascadeType != null) {
                    if (this.cascadeType.equals(CascadeType.PERSIST)) {
                        ...
                        if (dc != null)
                            DbUtils.updateCaches(cacheManager, d, Poolable.class);
                        if (ldc != null)
                            DbUtils.updateCaches(cacheManager, d, Poolable.class);
                    }
                }
    ...
    public List<Pool<? extends Poolable>> listAllByPlatformAndSearch(PlatformType platformType, String query) throws IOException {
        ...
        return template
            .query(POOL_SELECT_BY_PLATFORM_AND_SEARCH, new Object[] { platformType.getKey(), mySQLQuery, mySQLQuery, mySQLQuery },
                   new PoolMapper());
    }
    ...
    public List<Pool<? extends Poolable>> listReadyByPlatformAndSearch(PlatformType platformType, String query) throws IOException {
        String mySQLQuery = "%" + query + "%";
        return template
            .query(POOL_SELECT_BY_PLATFORM_AND_READY_AND_SEARCH, new Object[] { platformType.getKey(), mySQLQuery, mySQLQuery, mySQLQuery },
                   new PoolMapper());
    }
