...
public class SQLRunDAO implements RunStore {
    ...
    public static final String RUNS_SELECT_BY_SEARCH = RUNS_SELECT + " WHERE " +
                                                       "name LIKE ? OR " +
                                                       "alias LIKE ? OR " +
                                                       "description LIKE ? ";
    ...
    public static String RUNS_SELECT_BY_PROJECT_ID = "SELECT DISTINCT ra.* " +
                                                     "FROM Project p " +
    ...
    public static String RUNS_SELECT_BY_POOL_ID = "SELECT DISTINCT ra.* " +
                                                  "FROM Pool pool " +
                                                  "LEFT JOIN _Partition c ON pool.poolId = c.pool_poolId " +
                                                  "LEFT JOIN SequencerPartitionContainer_Partition fc ON c.partitionId = fc.partitions_partitionId " +
    ...
    public static String RUNS_SELECT_BY_SEQUENCER_PARTITION_CONTAINER_ID = "SELECT ra.* " +
                        ...
                        if (namingScheme.validateField("name", run.getName())) {
                            ...
                            if (newId.longValue() != run.getId()) {
                                ...
                                throw new IOException(
                                    "Something bad happened. Expected Run ID doesn't match returned value from DB insert");
                            }
                    ...
                    if (this.cascadeType.equals(CascadeType.PERSIST)) {
                        for (SequencerPartitionContainer<SequencerPoolPartition> l : run.getSequencerPartitionContainers()) {
                            ...
                            try {
                                ...
                                log.debug(
                                    "This Run/SequencerPartitionContainer combination already exists - not inserting: " + dke.getMessage());
                            }
                        }
                    }
