...
public class SQLRunQCDAO implements RunQcStore {
    ...
    public static final String RUN_QC = "SELECT qcId, run_runId, qcUserName, qcDate, qcMethod, information, doNotProcess " +
                                        "FROM " + TABLE_NAME;

    public static final String RUN_QC_SELECT_BY_ID = RUN_QC + " WHERE qcId=?";

    public static final String RUN_QC_SELECT_BY_RUN_ID = RUN_QC + " WHERE run_runId=? " +
                                                         "ORDER BY qcDate ASC";
    ...
    public long save(RunQC runQC) throws IOException {
        ...
        params.addValue("run_runId", runQC.getRun().getId()).addValue("qcUserName", runQC.getQcCreator())
              .addValue("qcDate", runQC.getQcDate()).addValue("qcMethod", runQC.getQcType().getQcTypeId())
              .addValue("information", LimsUtils.findHyperlinks(runQC.getInformation())).addValue("doNotProcess", runQC.getDoNotProcess());
