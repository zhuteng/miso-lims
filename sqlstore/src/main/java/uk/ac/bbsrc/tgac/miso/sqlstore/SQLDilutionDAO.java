...
public class SQLDilutionDAO implements DilutionStore {
    public static String DILUTION_SELECT_BY_ID_AND_LIBRARY_PLATFORM = "SELECT DISTINCT * " +
                                                                      "FROM Library l " +
    ...
    public Collection<LibraryDilution> listAllLibraryDilutionsByPlatform(PlatformType platformType) throws IOException {
        return template
            .query(LIBRARY_DILUTION_SELECT_BY_LIBRARY_PLATFORM, new Object[] { platformType.getKey() }, new LazyLibraryDilutionMapper());
    }
    ...
    public Collection<emPCRDilution> listAllEmPcrDilutionsByPlatform(PlatformType platformType) throws IOException {
        return template
            .query(EMPCR_DILUTION_SELECT_BY_LIBRARY_PLATFORM, new Object[] { platformType.getKey() }, new LazyEmPCRDilutionMapper());
    }
    ...
    public Collection<? extends Dilution> listAllDilutionsByProjectAndPlatform(long projectId, PlatformType platformType)
        throws IOException {
        if (platformType.equals(PlatformType.ILLUMINA)) {
            return template
                .query(LIBRARY_DILUTION_SELECT_BY_PROJECT_AND_LIBRARY_PLATFORM, new Object[] { platformType.getKey(), projectId },
            ...
            dils.addAll(template
                            .query(EMPCR_DILUTION_SELECT_BY_PROJECT_AND_LIBRARY_PLATFORM, new Object[] { platformType.getKey(), projectId },
    ...
    public long saveLibraryDilution(LibraryDilution dilution) throws IOException {
        ...
        if (dilution.getId() == AbstractDilution.UNSAVED_ID) {
            SimpleJdbcInsert insert = new SimpleJdbcInsert(template).withTableName("LibraryDilution")
                ...
                if (namingScheme.validateField("name", dilution.getName())) {
                    ...
                    if (newId.longValue() != dilution.getId()) {
                        ...
                        throw new IOException(
                            "Something bad happened. Expected LibraryDilution ID doesn't match returned value from DB insert");
                    }
                } else {
            ...
            try {
                ...
                if (namingScheme.validateField("name", dilution.getName())) {
                    ...
                    if (newId.longValue() != dilution.getId()) {
                        ...
                        new NamedParameterJdbcTemplate(template)
                            .update(EMPCR_DILUTION_DELETE, new MapSqlParameterSource().addValue("dilutionId", newId.longValue()));
                        throw new IOException(
                            "Something bad happened. Expected emPCRDilution ID doesn't match returned value from DB insert");
                    }
                } else {
