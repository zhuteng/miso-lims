...
public class SQLSecurityProfileDAO implements Store<SecurityProfile> {
    ...
    public static final String PROFILES_SELECT = "SELECT profileId, allowAllInternal, owner_userId " +
                                                 "FROM " + TABLE_NAME;
    ...
    public long save(SecurityProfile securityProfile) throws IOException {
        ...
        if (securityProfile.getProfileId() != SecurityProfile.UNSAVED_ID) {
            ...
            List<SecurityProfile> results = template
                .query(PROFILE_SELECT_BY_ID, new Object[] { securityProfile.getProfileId() }, new SecurityProfileMapper());
            if (results.size() > 0) {
