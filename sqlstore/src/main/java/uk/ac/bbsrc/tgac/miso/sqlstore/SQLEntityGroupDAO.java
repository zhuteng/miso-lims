...
public class SQLEntityGroupDAO implements EntityGroupStore {
    ...
    public static final String ENTITYGROUP_SELECT = "SELECT entityGroupId, parentId, parentType " +
                                                    "FROM " + TABLE_NAME;
    ...
    public EntityGroup<? extends Nameable, ? extends Nameable> lazyGet(long groupId) throws IOException {
        List<EntityGroup<? extends Nameable, ? extends Nameable>> eResults = template
            .query(ENTITYGROUP_SELECT_BY_ID, new Object[] { groupId }, new EntityGroupMapper(true));
        ...
        if (entityGroup.getId() == EntityGroupImpl.UNSAVED_ID) {
            ...
            if (newId.longValue() != entityGroup.getId()) {
                ...
                new NamedParameterJdbcTemplate(template)
                    .update(ENTITYGROUP_DELETE, new MapSqlParameterSource().addValue("entityGroupId", newId.longValue()));
    ...
    public EntityGroup<? extends Nameable, ? extends Nameable> get(long id) throws IOException {
        List<EntityGroup<? extends Nameable, ? extends Nameable>> eResults = template
            .query(ENTITYGROUP_SELECT_BY_ID, new Object[] { id }, new EntityGroupMapper());
        return eResults.size() > 0 ? eResults.get(0) : null;
    }
