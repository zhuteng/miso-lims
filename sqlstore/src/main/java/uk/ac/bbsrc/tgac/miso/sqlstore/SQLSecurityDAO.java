...
public class SQLSecurityDAO implements SecurityStore {
    public static final String USERS_SELECT =
        "SELECT userId, active, admin, external, fullName, internal, loginName, roles, password, email " + "FROM User";
    ...
    public static final String GROUPS_SELECT_BY_USER_ID = "SELECT g.groupId, g.name, g.description " +
                                                          "FROM _Group g, User_Group ug " +
                                                          "WHERE g.groupId=ug.groups_groupId " +
                                                          "AND ug.users_userId=?";

    public static final String GROUP_UPDATE = "UPDATE _Group " +
    ...
    public long saveUser(User user) throws IOException {
        ...
        if (user.getRoles() != null) {
            ...
            if (user.isExternal() && !roles.contains("ROLE_EXTERNAL"))
                roles.add("ROLE_EXTERNAL");
            if (user.isInternal() && !roles.contains("ROLE_INTERNAL"))
                roles.add("ROLE_INTERNAL");
            if (user.isAdmin() && !roles.contains("ROLE_ADMIN"))
