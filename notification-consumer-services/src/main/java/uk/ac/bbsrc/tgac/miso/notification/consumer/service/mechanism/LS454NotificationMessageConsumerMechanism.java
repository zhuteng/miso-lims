                        ...
                        if (attemptRunPopulation) {
                            if (r == null) {
                                ...
                                if (r.getStatus() != null && run.has("status")) {
                                    if (!r.getStatus().getHealth().equals(HealthType.Failed) &&
                                        !r.getStatus().getHealth().equals(HealthType.Completed)) {
                                ...
                                if (!r.getStatus().getHealth().equals(HealthType.Completed) &&
                                    ...
                                    log.debug(
                                        "Saving previously saved status: " + is.getRunName() + " (" + r.getStatus().getHealth().getKey() +
                                        " -> " + is.getHealth().getKey() + ")");
                                    r.setStatus(is);
                                }
                            }
                            ...
                            if (r.getSequencerReference() != null) {
                                if (run.has("runparams")) {
                                    try {
                                        ...
                                        List<SequencerPartitionContainer<SequencerPoolPartition>> fs = ((LS454Run) r)
                                            .getSequencerPartitionContainers();
                                        ...
                                        if (fs.isEmpty()) {
                                            if (ptp.getElementsByTagName("padLayout").getLength() > 0 &&
                                                ptp.getElementsByTagName("padLayout").item(0) != null) {
                                                int numPartitions = Integer
                                                    .parseInt(ptp.getElementsByTagName("padLayout").item(0).getTextContent().split("_")[0]);
                                                ...
                                                log.debug("\\_ Created new SequencerPartitionContainer with " + f.getPartitions().size() +
                                                          " partitions");
