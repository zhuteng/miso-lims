            ...
            if (run.has("cells")) {
                ...
                for (JSONObject cell : (Iterable<JSONObject>) cells) {
                    if (cell.has("cellStatus")) {
                        try {
                            String s = new String(
                                IntegrationUtils.decompress(URLDecoder.decode(cell.getString("cellStatus"), "UTF-8").getBytes()));
                            status += s + "\n\n";
                ...
                try {
                    ...
                    if (!status.startsWith("ERROR")) {
                        ...
                        if (attemptRunPopulation) {
                            if (r == null) {
                                ...
                                if (r.getSequencerReference() != null) {
                                    ...
                                    if (!r.getStatus().getHealth().equals(HealthType.Completed) &&
                                        ...
                                        log.info("Saving previously saved status: " + is.getRunName() + " (" +
                                                 r.getStatus().getHealth().getKey() + " -> " + is.getHealth().getKey() + ")");
                            ...
                            if (r.getSequencerReference() != null) {
                                List<SequencerPartitionContainer<SequencerPoolPartition>> fs = ((PacBioRun) r)
