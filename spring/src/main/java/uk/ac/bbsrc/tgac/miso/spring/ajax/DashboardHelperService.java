        ...
        if (username != null && !username.equals("")) {
            if (SecurityContextHolder.getContext().getAuthentication().getName().equals(username)) {
                try {
                    ...
                    e.printStackTrace();
                    return JSONUtils
                        .SimpleJSONError("Something went wrong trying to get user information from the database: " + e.getMessage());
                }
            } else {
    ...
    public JSONObject getAlerts(HttpSession session, JSONObject json) {
        ...
        try {
            ...
            for (Alert a : alerts) {
                ...
                if (!a.getAlertRead()) {
                    b.append(
                        "<span onclick='Utils.alert.confirmAlertRead(this);' class='float-right ui-icon ui-icon-circle-close'></span>");
                }
                b.append("</div>");
            }
        } catch (IOException e) {
