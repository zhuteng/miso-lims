        ...
        try {
            ...
            if (json.has("runId") && json.get("runId") != null) {
                ...
                if (r != null) {
                    ...
                    e.setTitle(
                        s.getProject().getName() + " " + r.getPlatformType().getKey() + " " + s.getStudyType() + " experiment (Auto-gen)");
                    ...
                    sb.append("<i>");
                    sb.append("<span>" + s.getProject().getAlias() + " (" + e.getName() + ": " + p.getDilutions().size() +
                              " dilutions)</span><br/>");
                    sb.append("</i>");
                ...
                for (Poolable p : ds) {
                    if (p instanceof Dilution) {
                        ...
                        info.append(
                            ...
                            " (" + dilution.getLibrary().getName() + ")</a>");
                        info.append("<br/><a href='/miso/sample/" + dilution.getLibrary().getSample().getId() + "'>" +
                                    dilution.getLibrary().getSample().getDescription() + " (" +
                                    dilution.getLibrary().getSample().getName() + ")</a></small>");
                        info.append("</li>");
                    } else if (p instanceof Plate) {
                        Plate<LinkedList<Plateable>, Plateable> plate = (Plate<LinkedList<Plateable>, Plateable>) p;
                        info.append("<li><b><a href='/miso/plate/" + plate.getId() + "'>" + plate.getName() + "</b> [" + plate.getSize() +
                                    "-well]</a>");
                        if (!plate.getElements().isEmpty()) {
                            info.append(
                                "<br/><small><u>" + plate.getSize() + "-well " + plate.getElementType().getSimpleName() + " plate</u>");
                            ...
                            if (element instanceof Library) {
                                ...
                                info.append("<br/><small><u><a href='/miso/project/" + l.getSample().getProject().getId() + "'>" +
                                            l.getSample().getProject().getAlias() + " (" + l.getSample().getProject().getName() +
                                            ")</a></u>");
                                ...
                                info.append(
                                    ...
                                    l.getLibrary().getSample().getProject().getAlias() + " (" +
                                    l.getLibrary().getSample().getProject().getName() + ")</a></u>");
                        ...
                        if (p instanceof Plate) {
                            ...
                            for (Plateable plateable : plate.getElements()) {
                                if (plateable instanceof Plate) {
                                    info.append("<b>Internal Plate:</b> <a href='/miso/plate/" + plate.getId() + "'>" + plate.getName() +
                                                "</a><br/>");
                                } else if (p instanceof Library) {
                                    Library library = (Library) plateable;
                                    info.append(
                                        "<b>Library:</b> <a href='/miso/library/" + library.getId() + "'>" + library.getAlias() + "(" +
                            ...
                            info.append("<b>Sample:</b> <a href='/miso/sample/" + dilution.getLibrary().getSample().getId() + "'>" +
                                        dilution.getLibrary().getSample().getAlias() + "(" + dilution.getLibrary().getSample().getName() +
                                        ")</a><br/>");
        ...
        if (json.has("platform") && !"".equals(json.getString("platform"))) {
            try {
                ...
                for (LibraryDilution libraryDilution : requestManager.listAllLibraryDilutionsByPlatform((PlatformType.get(platform)))) {
                    ...
                    pout.add(
                        libraryDilution.getLibrary().getSample().getName() + "-" + libraryDilution.getLibrary().getSample().getAlias());
