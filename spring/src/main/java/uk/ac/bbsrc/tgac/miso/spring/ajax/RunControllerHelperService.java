...
public class RunControllerHelperService {
            ...
            if (json.has("runId") && !json.getString("runId").equals("")) {
                ...
                if (newPt != null) {
                    ...
                    for (SequencerReference sr : requestManager.listSequencerReferencesByPlatformType(newPt)) {
                        srb.append("<option value='" + sr.getId() + "'>" + sr.getName() + " (" + sr.getPlatform().getInstrumentModel() +
                                   ")</option>");
                    }
                    srb.append("</select>");
        ...
        if ("Illumina MiSeq".equals(instrumentModel) || "Illumina NextSeq 500".equals(instrumentModel)) {
            ...
            for (int i = 0; i < numContainers; i++) {
                ...
                b.append("<tr><td>ID:</td><td><button onclick='Run.container.lookupContainer(this, " + i +
                         ...
                         i + "].identificationBarcode' name='sequencerPartitionContainers[" + i +
                         "].identificationBarcode'/></div></td></tr>");
                ...
                for (SequencerPartitionContainer<SequencerPoolPartition> f : ((RunImpl) r).getSequencerPartitionContainers()) {
                    ...
                    for (Partition p : f.getPartitions()) {
                        sb.append("<td onclick='Run.qc.toggleProcessPartition(this);' runId='" + r.getId() + "' containerId='" + f.getId() +
                                  "' partitionNumber='" + p.getPartitionNumber() + "' id='" + r.getId() + "_" + f.getId() + "_" +
                                  p.getPartitionNumber() + "' class='smallbox'>" + p.getPartitionNumber() + "</td>");
                    }
                    sb.append("</tr></table>");
                }
                    ...
                    for (SequencerPartitionContainer<SequencerPoolPartition> f : ((RunImpl) run).getSequencerPartitionContainers()) {
                        ...
                        for (Partition p : f.getPartitions()) {
                            if (processSelections.contains(run.getId() + "_" + f.getId() + "_" + p.getPartitionNumber())) {
                                sb.append(
                                    "<td runId='" + run.getId() + "' containerId='" + f.getId() + "' partitionId='" + p.getId() + "' id='" +
  ...
  */
            ...
            try {
                ...
                if (!fs.isEmpty()) {
                    ...
                    if (fs.size() == 1) {
                        ...
                        SequencerPartitionContainer<SequencerPoolPartition> f = new ArrayList<SequencerPartitionContainer<SequencerPoolPartition>>(
                            fs).get(0);
                        ...
                        for (SequencerPoolPartition p : f.getPartitions()) {
                            ...
                            if (p.getPool() != null) {
                                ...
                                sb.append("<ul partition='" + (p.getPartitionNumber() - 1) + "' bind='sequencerPartitionContainers[" +
                                          containerNum + "].partitions[" + (p.getPartitionNumber() - 1) +
                                ...
                                if (!p.getPool().getExperiments().isEmpty()) {
                                    ...
                                    sb.append("<input type='hidden' name='sequencerPartitionContainers[" + containerNum + "].partitions[" +
                                              (p.getPartitionNumber() - 1) + "].pool' id='pId" + (p.getPartitionNumber() - 1) +
                                              "' value='" + p.getPool().getId() + "'/>");
                                } else {
                                ...
                                sb.append("<ul class='runPartitionDroppable' bind='sequencerPartitionContainers[" + containerNum +
                                          "].partitions[" + (p.getPartitionNumber() - 1) + "].pool' partition='" +
                                          (p.getPartitionNumber() - 1) + "' ondblclick='Run.container.populatePartition(this);'></ul>");
                                sb.append("</div>");
                            }
                            sb.append("</td>");
                            sb.append("</tr>");
    ...
    public JSONObject generateIlluminaDemultiplexCSV(HttpSession session, JSONObject json) throws IOException {
        ...
        SequencerPartitionContainer<SequencerPoolPartition> f = requestManager
            .getSequencerPartitionContainerById(json.getLong("containerId"));
        if (r != null && f != null) {
        ...
        try {
            ...
            for (Project project : pooledProjects) {
                ...
                sb.append("<input id='studySelectButton-" + partition + "_" + p.getId() +
                          "' type='button' onclick=\"Run.container.selectStudy('" + partition + "', " + p.getId() + "," +
    ...
    private String poolHtml(Pool<? extends Poolable> p, int container, int partition) {
        ...
        try {
            ...
            b.append("</div>");
            b.append(
                "<input type='hidden' name='sequencerPartitionContainers[" + container + "].partitions[" + partition + "].pool' id='pId" +
