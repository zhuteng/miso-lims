            ...
            try {
                if (libraryNamingScheme.validateField("alias", alias)) {
                    ...
                    return JSONUtils.SimpleJSONError("The following Library alias doesn't conform to the chosen naming scheme (" +
                                                     libraryNamingScheme.getValidationRegex("alias") + ") or already exists: " +
    ...
    public JSONObject changeLibraryLocation(HttpSession session, JSONObject json) {
        ...
        try {
            ...
            if (newLocation != null) {
                ...
                note.setText(
                    "Location changed from " + oldLocation + " to " + newLocation + " by " + user.getLoginName() + " on " + new Date());
                note.setOwner(user);
                note.setCreationDate(new Date());
                library.getNotes().add(note);
                requestManager.saveLibraryNote(library, note);
                  ...
                  if (libraryNamingScheme.validateField("alias", la)) {
                    libAlias = la;
                        ...
                        if (sample != null) { // && libAlias != null) {
                            ...
                            if (j.has("tagBarcodes") && !"".equals(j.getString("tagBarcodes")) &&
                                ...
                                for (String code : codes) {
                                    try {
                                        ...
                                        return JSONUtils.SimpleJSONError(
                                            "Cannot save Library. It looks like there are tag barcodes for the library of " +
                        ...
                        e.printStackTrace();
                        return JSONUtils
                            .SimpleJSONError("Cannot save Library generated from " + j.getString("parentSample") + ": " + e.getMessage());
                    } catch (JSONException e) {
                        e.printStackTrace();
                        return JSONUtils.SimpleJSONError(
                            "Cannot save Library. Something cannot be retrieved from the bulk input table: " + e.getMessage());
                    }
                }
        ...
        try {
            ...
            if (json.has("libraryId") && !json.get("libraryId").equals("")) {
                ...
                for (LibraryDilution dil : library.getLibraryDilutions()) {
                    ...
                    if (!library.getPlatformName().equals("Illumina")) {
                        sb.append("<td><a href='javascript:void(0);' onclick='Library.empcr.insertEmPcrRow(" + dil.getId() +
                                  ");'>Add emPCR</a></td>");
                    } else {
        ...
        try {
            ...
            if (json.has("pcrId") && !json.get("pcrId").equals("")) {
                ...
                for (emPCRDilution dil : requestManager.listAllEmPcrDilutionsByEmPcrId(pcrId)) {
                    ...
                    sb.append("<td><a href='/miso/poolwizard/new/" +
                              pcr.getLibraryDilution().getLibrary().getSample().getProject().getProjectId() +
                              "'>Construct New Pool</a></td>");
                    sb.append("</tr>");
                }
                return JSONUtils.SimpleJSONResponse(sb.toString());
    ...
    public JSONObject bulkAddEmPcrs(HttpSession session, JSONObject json) {
        try {
            ...
            if (ok) {
                ...
                log.error(
                    "Failed to add EmPCRs to this Library Dilution: one of the required fields of the selected EmPCR is missing or invalid");
                return JSONUtils.SimpleJSONError(
