...
public class ContainerControllerHelperService {
    ...
    private String getContainerOptions(SequencerReference sr) throws IOException {
        ...
        for (int i = 0; i < sr.getPlatform().getNumContainers(); i++) {
            b.append("<input id='container" + (i + 1) + "' name='containerselect' onchange='Container.ui.changeContainer(" +
                     sr.getPlatform().getNumContainers() + ", \"" + pt.getKey() + "\", " + sr.getId() + ");' type='radio' value='" +
                     (i + 1) + "'/>" + (i + 1));
        }
        b.append("</span><br/>");
    ...
    public JSONObject changeIlluminaLane(HttpSession session, JSONObject json) {
        ...
        for (int i = 0; i < numLanes; i++) {
            ...
            b.append("<td width='90%'><div id='p_div_" + container + "-" + i +
                     "' class='elementListDroppableDiv'><ul class='runPartitionDroppable' bind='partitions[" + i + "].pool' partition='" +
                     i + "' ondblclick='Container.partition.populatePartition(this);'>");
            b.append("</ul></div></td>");
            b.append("</tr>");
        }
        b.append("</table>");
        ...
        try {
                ...
                if (d instanceof Dilution) {
                    ...
                    if (!plate.getElements().isEmpty()) {
                        ...
                        if (element instanceof Library) {
                            ...
                            b.append("<span>" + dl.getName() + " [" + plate.getSize() + "-well] (" +
                                     dl.getLibrary().getSample().getProject().getAlias() + ")</span><br/>");
                        } else if (element instanceof Sample) {
                            Sample s = (Sample) element;
                            b.append(
                                "<span>" + s.getName() + " [" + plate.getSize() + "-well] (" + s.getProject().getAlias() + ")</span><br/>");
                        }
                    }
                } else {
            ...
            if (p.getExperiments().size() == 0) {
                ...
                for (Project project : pooledProjects) {
                    ...
                    b.append(project.getAlias() + ": <select name='poolStudies" + partition + "_" + project.getProjectId() +
                             "' id='poolStudies" + partition + "_" + project.getProjectId() + "'>");
                    Collection<Study> studies = requestManager.listAllStudiesByProjectId(project.getProjectId());
                    if (studies.isEmpty()) {
                        ...
                        for (Study s : studies) {
                            b.append("<option value='" + s.getId() + "'>" + s.getAlias() + " (" + s.getName() + " - " + s.getStudyType() +
                                     ")</option>");
                        }
                    }
                    b.append("</select>");
            ...
            b.append("<input type='hidden' name='partitions[" + partition + "].pool' id='pId" + p.getId() + "' value='" + p.getId() +
                     "'/></div>");
    ...
    public JSONObject selectStudyForPool(HttpSession session, JSONObject json) {
        try {
            ...
            e.setTitle(
                s.getProject().getName() + " " + platform.getPlatformType().getKey() + " " + s.getStudyType() + " experiment (Auto-gen)");
            ...
            sb.append("<i>");
            sb.append(
                "<span>" + s.getProject().getAlias() + " (" + e.getName() + ": " + p.getDilutions().size() + " dilutions)</span><br/>");
            sb.append("</i>");
                    ...
                    if (fs.size() == 1) {
                        ...
                        for (SequencerPoolPartition p : f.getPartitions()) {
                            ...
                            if (p.getPool() != null) {
                                ...
                                sb.append("<ul partition='" + (p.getPartitionNumber() - 1) + "' bind='partitions[" +
                                          (p.getPartitionNumber() - 1) + "].pool' class='runPartitionDroppable'>");
                                ...
                                if (!p.getPool().getExperiments().isEmpty()) {
                                    ...
                                    for (Experiment e : p.getPool().getExperiments()) {
                                        sb.append(e.getStudy().getProject().getAlias() + " (" + e.getName() + ": " +
                                                  p.getPool().getDilutions().size() + " dilutions)<br/>");
                                    }
                                    sb.append("</i>");
                                ...
                                sb.append("<ul class='runPartitionDroppable' bind='partitions[" + (p.getPartitionNumber() - 1) +
                                          "].pool' partition='" + (p.getPartitionNumber() - 1) +
                                          "' ondblclick='Container.partition.populatePartition(this);'></ul>");
                                sb.append("</div>");
                            }
                            sb.append("</td>");
                            sb.append("</tr>");
                ...
                if (sequencePartitionContainer.getRun() != null) {
                    ...
                    if (sequencePartitionContainer.getRun().getSequencerReference() != null) {
                        sequencer = "<a href=\"/miso/sequencer/" + sequencePartitionContainer.getRun().getSequencerReference().getId() +
                                    "\">" + sequencePartitionContainer.getRun().getSequencerReference().getPlatform().getNameAndModel() +
                                    "</a>";
                    }
                }

                jsonArray.add("['" +
                              (sequencePartitionContainer.getIdentificationBarcode() != null ?
                               sequencePartitionContainer.getIdentificationBarcode() :
                               "") + "','" +
                              (sequencePartitionContainer.getPlatform() != null &&
                               sequencePartitionContainer.getPlatform().getPlatformType() != null ?
                               sequencePartitionContainer.getPlatform().getPlatformType().getKey() :
                               "") + "','" +
                              run + "','" +
                              sequencer + "','" +
                              "<a href=\"/miso/container/" + sequencePartitionContainer.getId() +
