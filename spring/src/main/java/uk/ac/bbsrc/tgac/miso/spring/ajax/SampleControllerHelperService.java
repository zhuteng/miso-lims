            ...
            try {
                if (sampleNamingScheme.validateField("alias", alias)) {
                    ...
                    return JSONUtils.SimpleJSONError("The following sample alias doesn't conform to the chosen naming scheme (" +
                                                     sampleNamingScheme.getValidationRegex("alias") + ") or already exists: " +
            ...
            try {
                ...
                if (complement != null && !complement.isEmpty()) {
                    ...
                    return JSONUtils.SimpleJSONError(
                        "Error in saving samples - perhaps samples specified already exist in the database with a given alias?");
                }
        ...
        try {
            ...
            if (serviceName == null) {
                ...
                if (services.size() == 1) {
                    ...
                    if (mps == null) {
                        return JSONUtils.SimpleJSONError(
                            "Unable to resolve a print service for Samples. A service seems to be recognised but cannot be resolved.");
                    }
                } else {
                    return JSONUtils
