    ...
    public JSONObject submitSequenceData(HttpSession session, JSONObject json) {
        try {
            if (json.has("submissionId") && !json.get("submissionId").equals("")) {
                ...
                return JSONUtils
                    .SimpleJSONError("Failed to get submission report. Something went wrong in the sequence Data submission process");
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            if (json.has("projectId") && !json.get("projectId").equals("")) {
                ...
                if (runs.size() > 0) {
                    ...
                    for (Run r : runs) {
                        ...
                        sb.append(
                            "<input type='hidden' id='RUN_" + r.getId() + "' name='RUN_" + r.getId() + "' value='" + r.getId() + "'/>");
                        sb.append("<ul>");
                                ...
                                if (part.getPool() != null) {
                                    ...
                                    if (partitionInvolved) {
                                        ...
                                        sb.append("<li><input type='checkbox' id='" + r.getId() + "_" + partitionContainer.getId() + "_" +
                                                  part.getPartitionNumber() + "' name='partition' " +
                                                  "itemLabel='" + part.getPartitionNumber() + "' itemValue='PAR" + part.getId() +
                                                  "' value='PAR" + part.getId() +
                                                  "' onclick='Submission.ui.togglePartitionContents(this)'");

                                        if (sub != null) {
                                            // checks checkboxes if the partition is in the submission
                                            if (sub != null && !sub.getSubmissionElements().isEmpty() &&
                                                sub.getSubmissionElements().contains(part)) {
                                        ...
                                        for (Poolable d : poolables) {
                                            sb.append(
                                                "<li><input type='checkbox'  name='DIL_" + d.getId() + "' id='DIL" + d.getId() + "_PAR" +
                                                part.getId() + "' value='PAR_" + part.getId() + "' ");
                                            ...
                                            if (d instanceof Dilution) {
                                                sb.append(
                                                    ">" + partitionContainer.getId() + "_" + ((Dilution) d).getLibrary().getName() + "_" +
                                                    d.getName() + ": ");
                                                sb.append("<ul>");
                                                try {
