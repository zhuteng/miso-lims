...
public class FlexReportingControllerHelperService {
    ...
    public String projectRowBuilder(Project project) {
        return "['<input class=\"chkboxprojects\" id=\"" + project.getProjectId() +
               ...
               "\"/>','" + project.getName() + "','" + project.getAlias() + "','" + project.getDescription() + "','" +
                                    ...
                                    if (spp.getPool() != null) {
                                        if (spp.getPool().getDilutions().size() > 0) {
                                            for (Dilution dilution : spp.getPool().getDilutions()) {
                                                ...
                                                if (libraryInRun.getSample().getProject()
                                                                .equals(requestManager.getProjectById(project.getProjectId()))) {
                ...
                for (Run run : runs) {
                    ...
                    if (from != null && !from.equals("") &&
                        ...
                        if (run.getStatus() != null && run.getStatus().getCompletionDate() != null) {
                            ...
                            if ((runDate.after(startDate) && runDate.before(endDate)) || runDate.equals(startDate) ||
                                runDate.equals(endDate)) {
                                projectBool = true;
                            }
                        }
                    } else {
    ...
    public String projectRunLaneRowBuilder(Project project) {
        ...
        try {
            ...
            return "['<input id=\"" + project.getProjectId() +
                   ...
                   project.getAlias() + "','" + project.getProgress().name() + "','" + sb.toString() + "']";
        } catch (IOException e) {
            log.debug("Failed", e);
                                            ...
                                            if (libraryInRun.getSample().getProject()
                                                ...
                                                if (librariesInRun.add(libraryInRun)) {
                                                    ...
                                                    for (Map.Entry<Integer, TagBarcode> entry : libraryInRun.getTagBarcodes().entrySet()) {
                                                        tagBarcode
                                                            .append(entry.getKey().toString() + ": " + entry.getValue().getName() + " (" +
    ...
    public String sampleFormRowBuilder(Sample sample) {
        ...
        return "['<input class=\"chkboxsamples\" id=\"" + sample.getId() +
               ...
               sample.getName() + "','" + sample.getAlias() + "','" + sample.getDescription() + "','" + sample.getSampleType() + "','" +
               qc + "']";
    }
    ...
    public String libraryFormRowBuilder(Library library) {
        ...
        return "['<input class=\"chkboxlibraries\" id=\"" + library.getId() +
               ...
               library.getName() + "','" + library.getAlias() + "','" + library.getDescription() + "','" + library.getPlatformName() +
            ...
            for (JSONObject j : (Iterable<JSONObject>) a) {
                if (j.getString("name").equals("libraryIds")) {
                    ...
                    if (l != null) {
                        ...
                        int count = typeMap.containsKey(l.getLibraryType().getDescription()) ?
                                    typeMap.get(l.getLibraryType().getDescription()) :
                                    0;
                        count++;
    ...
    public String runFormRowBuilder(Run run) {
        return "['<input class=\"chkboxruns\" id=\"" + run.getId() + "\" type=\"checkbox\" name=\"runIds\" value=\"" + run.getId() +
               "\" id=\"" + run.getId() + "\"/>','" + run.getName() + "','" + run.getAlias() + "','" +
    ...
    public JSONObject searchRunsByCreationDateandString(HttpSession session, JSONObject json) {
        ...
        try {
            ...
            for (Run run : runs) {
                if ((platform.equals("all") || platform.equals(run.getPlatformType().getKey())) && (status.equals("all") ||
                                                                                                    (run.getStatus() != null &&
                                                                                                     run.getStatus().getHealth() != null ?
                                                                                                     status.equals(
                                                                                                         run.getStatus().getHealth()
                                                                                                            .getKey()) :
                                                                                                     true))) {
            ...
            for (JSONObject j : (Iterable<JSONObject>) a) {
                if (j.getString("name").equals("runIds")) {
                    ...
                    if (run != null) {
                        ...
                        int count = platformTypeMap.containsKey(run.getPlatformType().getKey()) ?
                                    platformTypeMap.get(run.getPlatformType().getKey()) :
                                    0;
                        count++;
                            ...
                            for (SequencerPoolPartition spp : spc.getPartitions()) {
                                if (spp.getPool() != null) {
                                    ...
                                    if (spp.getPool().getDilutions().size() > 0) {
                                        ...
                                        for (Dilution dilution : spp.getPool().getDilutions()) {
                                            if (!projectMapDisplayed
                                                .containsKey(dilution.getLibrary().getSample().getProject().getName())) {
        ...
        try {
            ...
            for (Run run : runs) {
                if (run.getStatus() != null && run.getStatus().getHealth() != null &&
                    ...
                    runsArray.add(
                        JSONObject.fromObject("{'name': '" + run.getName() + "','description':'" + run.getAlias() + "','color': '1'}"));
                } else {
                    runsArray.add(
                        JSONObject.fromObject("{'name': '" + run.getName() + "','description':'" + run.getAlias() + "','color': '0'}"));
                }
            }
            ...
            for (Study study : studies) {
                ...
                if (experiments.size() > 0) {
                    ...
                    for (Experiment e : experiments) {
                        experimentsArray.add(
                            JSONObject.fromObject("{'name': '" + e.getName() + "','description':'" + e.getAlias() + "','color': '2'}"));
                    }
                ...
                if (libraries.size() == 0) {
                    if (sample.getQcPassed()) {
                        samplesArray.add(JSONObject.fromObject(
                            "{'name': '" + sample.getName() + "','description':'" + sample.getAlias() + "','color': '1'}"));
                    } else {
                        samplesArray.add(JSONObject.fromObject(
                            "{'name': '" + sample.getName() + "','description':'" + sample.getAlias() + "','color': '0'}"));
                    }
                } else {
                    ...
                    for (Library library : libraries) {
                        if (library.getLibraryQCs().size() > 0) {
                            librariesArray.add(JSONObject.fromObject(
                                "{'name': '" + library.getName() + "','description':'" + library.getAlias() + "','color': '1'}"));
                        } else {
                            librariesArray.add(JSONObject.fromObject(
                                "{'name': '" + library.getName() + "','description':'" + library.getAlias() + "','color': '0'}"));
                        }
                    }
